    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            if (Request.QueryString["NoProduit"] != null)
            {
                ...
                tbPrixDemande.Text = string.Format("{0:N}", produit.PrixDemande).Replace(",", ".");
                tbDescription.Text = produit.Description;
                tbNbItems.Text = produit.NombreItems.ToString();
                tbPrixVente.Text = string.Format("{0:N}", produit.PrixVente).Replace(",", ".");
                tbDateVente.Text = produit.DateVente.Value.ToShortDateString();
                tbPoids.Text = string.Format("{0:N1}", produit.Poids).Replace(",", ".");
